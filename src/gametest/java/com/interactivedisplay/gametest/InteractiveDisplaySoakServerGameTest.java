...
public final class InteractiveDisplaySoakServerGameTest implements CustomTestMethodInvoker {
    ...
    public void windowAndGroupLifecycleShouldRemainCleanAcrossOneHundredCycles(GameTestHelper helper) {
        ...
        helper.assertTrue(manager.pooledEntityCount() == 0, Component.literal("display entity pool should remain disabled"));
        ...
        for (int i = 0; i < 100; i++) {
            ...
            var active = manager.findActiveWindow(player.getUUID(), "main_menu");
            helper.assertTrue(active != null, Component.literal("active window missing at iteration " + i));
            var oldHolder = active.virtualHolder();
            ...
            helper.assertTrue(oldHolder.entityCount() == 0, Component.literal("window virtual entities retained at iteration " + i));
            ...
            helper.assertTrue(manager.pooledEntityCount() == 0, Component.literal("display pool unexpectedly populated at iteration " + i));
        }
        ...
        for (int i = 0; i < 100; i++) {
            ...
            var active = manager.findActiveGroup(player.getUUID(), "menu_group");
            helper.assertTrue(active != null, Component.literal("active group missing at iteration " + i));
            var oldHolder = active.currentWindow().virtualHolder();
            ...
            VirtualWindowHolder.destroyAllPending(server);
            helper.assertTrue(oldHolder.entityCount() == 0, Component.literal("group virtual entities retained at iteration " + i));
            ...
            helper.assertTrue(manager.pooledEntityCount() == 0, Component.literal("display pool unexpectedly populated at group iteration " + i));
        }
